...
wrapper_setPostgresVersion() {
    ...
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    local postgresVer=${JFROG_POSTGRES_13_10X_VERSION}
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
...
_askElasticSearchDetails() {
    ...
    if [ "$isElasticSearchExternalised" == "$FLAG_Y" ]; then
       ...
       if [[ "$JF_NODE_TYPE" == "$NODE_TYPE_STANDALONE" ]]; then
           ...
           _updateFromSystemYaml "SHARED_ELASTICSEARCH_CERTIFICATE_PATH" "KEY_ELASTICSEARCH_CERTIFICATE_PATH"
           [ -z "$YAML_VALUE" ] && echo -n "Copy $ELASTICSEARCH_LABEL certificate of the first node [ path - {INSIGHT_DATA_DIR}/var/data/elasticsearch/config/certs ]" && wrapper_askUser "ELASTICSEARCH_CERTIFICATE_PATH" || logger "Found $PROMPT_ELASTICSEARCH_CERTIFICATE_PATH in system.yaml. Skipping prompt"
       fi
    fi
}
...
docker_setupModeSpecificVar() {
    ...
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        if [ -z "${PROJECT_ROOT_FOLDER_RT}" ]; then
            PROJECT_ROOT_FOLDER_RT=${JF_ROOT_DATA_DIR##*/}
        fi
        logDebug "PROJECT_ROOT_FOLDER_RT is [${PROJECT_ROOT_FOLDER_RT}]"

        if [ -z "${PROJECT_ROOT_FOLDER_XRAY}" ]; then
            PROJECT_ROOT_FOLDER_XRAY=${JF_ROOT_DATA_DIR##*/}
        fi
        logDebug "PROJECT_ROOT_FOLDER_XRAY is [${PROJECT_ROOT_FOLDER_XRAY}]"
    else
...
docker_processOptions() {
    #Determine the data folder to use for products
...
docker_showSuccessMessage() {
    ...
    if [[ "${DOCKER_DESKTOP_SETUP}" == "$FLAG_Y" ]]; then
      SERVER_URL="http://localhost:${JF_ROUTER_ENTRYPOINTS_EXTERNALPORT}"
    fi
...
_updateFromYaml () {
    ...
    if [ ! -z "$systemYaml" ] && [ -f "$systemYaml" ]; then
        _commonYamlProperties "$systemYaml"
        docker_hook_updateFromYaml "$systemYaml"
    fi
}
...
docker_updateFromYaml () {
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        SYSTEM_YAML_FILE_RT="${JF_ROOT_DATA_DIR_RT}/var/etc/${FILE_NAME_SYSTEM_YAML}"
        SYSTEM_YAML_FILE_XRAY="${JF_ROOT_DATA_DIR_XRAY}/var/etc/${FILE_NAME_SYSTEM_YAML}"
        _updateFromYaml "${SYSTEM_YAML_FILE_RT}"
        _updateFromYaml "${SYSTEM_YAML_FILE_XRAY}"
        _getYamlValueAndUpdateEnv "router.entrypoints.externalPort" "JF_ROUTER_ENTRYPOINTS_EXTERNALPORT" "${SYSTEM_YAML_FILE_RT}"
        _getYamlValueAndUpdateEnv "router.entrypoints.externalPort" "JF_XRAY_ROUTER_ENTRYPOINTS_EXTERNALPORT" "${SYSTEM_YAML_FILE_XRAY}"
    else
        SYSTEM_YAML_FILE="${JF_ROOT_DATA_DIR}/var/etc/${FILE_NAME_SYSTEM_YAML}"
        _updateFromYaml "${SYSTEM_YAML_FILE}"
    fi
}
...
docker_doValidations() {
    ...
    validationCheckPrerequisites
    if [[ ${TRIAL_FLOW} == "$FLAG_Y" ]]; then
        docker_validateDiskSpace "${JF_ROOT_DATA_DIR_RT}"
        docker_validateDiskSpace "${JF_ROOT_DATA_DIR_XRAY}"
    else   
        docker_validateDiskSpace "${JF_ROOT_DATA_DIR}"
    fi
...
docker_setRootDirPermissions() {
    ...
    local dockerUser="$2"
    local OS_TYPE=$(uname)
    local message="Aborting Installation. The folder [${rootDir}] should be owned by [${dockerUser}:${dockerUser}]. Please fix by running the command: [chown ${dockerUser}:${dockerUser} ${rootDir}]"

    # Fix directories ownerships
    io_setOwnershipNonRecursive "${rootDir}" "${dockerUser}" "${dockerUser}" || errorExit "$message"

    # Give wide permissions on Mac (to support the non-root Artifactory and Nginx containers)
    if [ "${OS_TYPE}" == "Darwin" ]; then
        logDebug "Setting 777 permissions on [${rootDir}]"
        chmod -R 777 "${rootDir}" || errorExit "Setting 777 permissions on [${rootDir}] failed"
    fi
}
...
docker_createDirectories() {
    ...
    createDir "${rootDir}/var" "${dockerUser}" "${dockerUser}"
    createDir "${rootDir}/var/etc" "${dockerUser}" "${dockerUser}"
    createDir "${rootDir}/var/data" "${dockerUser}" "${dockerUser}"
    createDir "${rootDir}/var/log" "${dockerUser}" "${dockerUser}"
    createDir "${rootDir}/var/etc/security" "${dockerUser}" "${dockerUser}"

    docker_hook_createDirectories
    
    logDebug "value of [FIRST_SETUP] is [$FIRST_SETUP]"
    if [ "$FIRST_SETUP" == true ]; then
        io_setOwnership "${rootDir}" "${dockerUser}" "${dockerUser}" || errorExit "Setting ownership of [${rootDir}] to [${dockerUser}:${dockerUser}] failed"
    fi
}
...
docker_installToSystemYaml() {
  ...
  local targetYaml="${4:-${SYSTEM_YAML_FILE}}"
...
docker_createSystemYaml() {
    ...
    if [[ "${DOCKER_DESKTOP_SETUP}" == "$FLAG_Y" ]]; then
        docker_setSystemValue "shared.node.ip" "${JFROG_HOST_DOCKER_INTERNAL}" "${systemYaml}"
    else
        docker_setSystemValue "shared.node.ip" "$JF_SHARED_NODE_IP" "${systemYaml}"
    fi
    ...
    if [ "$IS_DEVELOPMENT_MODE" == "$FLAG_Y" ]; then
        ...
        docker_setSystemValue "shared.node.id" "$(io_getPublicHostID)-${JF_PROJECT_NAME}" "${systemYaml}"
    else
        if [[ "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
            docker_setSystemValue "shared.node.id" "$(io_getPublicHostName)-xray" "${SYSTEM_YAML_FILE_XRAY}"
            docker_setSystemValue "shared.node.id" "$(io_getPublicHostName)-rt" "${SYSTEM_YAML_FILE_RT}"
        else
            docker_setSystemValue "shared.node.id" "$(io_getPublicHostName)" "${systemYaml}"
        fi
    fi

    if [[ "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
        docker_setSystemValue "shared.node.name" "$(io_getPublicHostName)-xray" "${SYSTEM_YAML_FILE_XRAY}"
        docker_setSystemValue "shared.node.name" "$(io_getPublicHostName)-rt" "${SYSTEM_YAML_FILE_RT}"
    else
        docker_setSystemValue "shared.node.name" "$(io_getPublicHostName)" "${systemYaml}"
    fi
    ...
    if [ ! -z "${JF_SHARED_JFROGURL}" ]; then
        if [[ "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
            ...
            docker_setSystemValue "shared.jfrogUrl" "${JF_SHARED_JFROGURL}" "${systemYaml}"
        fi
    fi

    if [ ! -z "${JF_SHARED_SECURITY_JOINKEY}" ]; then
        if [[ "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
            docker_setSystemValue "shared.security.joinKey" "${JF_SHARED_SECURITY_JOINKEY}" "${SYSTEM_YAML_FILE_XRAY}"
        else
            docker_setSystemValue "shared.security.joinKey" "${JF_SHARED_SECURITY_JOINKEY}" "${systemYaml}"
        fi
    fi
     
    # Mark wss flavour in system yaml and  add whitesource url
    if [ ! -z "$THIRD_PARTY_SCRIPT" ]; then
        docker_setSystemValue "${SYS_KEY_SERVER_WSS_BASE_URL_KEY}" "${WSS_SERVER_BASE_URL}" "${systemYaml}"
        docker_setSystemValue "${SYS_KEY_SERVER_INTEGRATION_ENABLE_KEY}" "true" "${systemYaml}"
    fi

    if [ ! -z "${JF_ROUTER_ENTRYPOINTS_EXTERNALPORT}" ]; then
        docker_setSystemValue "router.entrypoints.externalPort" "${JF_ROUTER_ENTRYPOINTS_EXTERNALPORT}" "${systemYaml}"
    fi

    if [ ! -z "${JF_XRAY_ROUTER_ENTRYPOINTS_EXTERNALPORT}" ]; then
        docker_setSystemValue "router.entrypoints.externalPort" "${JF_XRAY_ROUTER_ENTRYPOINTS_EXTERNALPORT}" "${SYSTEM_YAML_FILE_XRAY}"
    fi

    docker_installToSystemYaml "$KEY_DATABASE_TYPE"     "$FLAG_NOT_APPLICABLE" "${SYS_KEY_SHARED_DATABASE_TYPE}" "${systemYaml}" || logSilly "Key: KEY_DATABASE_TYPE not applicable"
    docker_installToSystemYaml "$KEY_DATABASE_DRIVER"   "$FLAG_NOT_APPLICABLE" "${SYS_KEY_SHARED_DATABASE_DRIVER}" "${systemYaml}" || logSilly "Key: KEY_DATABASE_DRIVER not applicable"    
    docker_installToSystemYaml "$KEY_DATABASE_URL"      "$FLAG_NOT_APPLICABLE" "${SYS_KEY_SHARED_DATABASE_URL}" "${systemYaml}" || logSilly "Key: KEY_DATABASE_URL not applicable"
    docker_installToSystemYaml "$KEY_DATABASE_USERNAME" "$FLAG_NOT_APPLICABLE" "${SYS_KEY_SHARED_DATABASE_USERNAME}" "${systemYaml}" || logSilly "Key: KEY_DATABASE_USERNAME not applicable"
    docker_installToSystemYaml "$KEY_DATABASE_PASSWORD" "$FLAG_NOT_APPLICABLE" "${SYS_KEY_SHARED_DATABASE_PASSWORD}" "${systemYaml}" || logSilly "Key: KEY_DATABASE_PASSWORD not applicable"
    docker_installToSystemYaml "$KEY_RABBITMQ_ACTIVE_NODE_NAME" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_RABBITMQ_ACTIVE_NODE_NAME" "${systemYaml}" || logSilly "Key: KEY_RABBITMQ_ACTIVE_NODE_NAME not applicable"
    docker_installToSystemYaml "$KEY_RABBITMQ_ACTIVE_NODE_IP" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_RABBITMQ_ACTIVE_NODE_IP" "${systemYaml}" || logSilly "Key: KEY_RABBITMQ_ACTIVE_NODE_IP not applicable"
    docker_installToSystemYaml "$KEY_REDIS_PASSWORD" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_SHARED_REDIS_PASSWORD" "${systemYaml}" || logSilly "Key: KEY_REDIS_PASSWORD not applicable"
    if [[ "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
        docker_installToSystemYaml "$KEY_RABBITMQ_PASSWORD" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_SHARED_RABBITMQ_PASSWORD" "${SYSTEM_YAML_FILE_XRAY}" || logSilly "Key: KEY_RABBITMQ_PASSWORD not applicable"
    else
        docker_installToSystemYaml "$KEY_RABBITMQ_PASSWORD" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_SHARED_RABBITMQ_PASSWORD" "${systemYaml}" || logSilly "Key: KEY_RABBITMQ_PASSWORD not applicable"
    fi
    ...
    if [[ ${TRIAL_FLOW} == "$FLAG_Y" ]]; then
        io_setOwnership "${SYSTEM_YAML_FILE_XRAY}" "$DOCKER_USER_XRAY" "$DOCKER_USER_XRAY"
        io_setOwnership "${SYSTEM_YAML_FILE_RT}" "$DOCKER_USER_RT" "$DOCKER_USER_RT"
    else
...
dockerCreateJoinKeyInRT() {
    if [ ! -z "${JF_SHARED_SECURITY_JOINKEY}" ]; then
        local joinKeyFile="${JF_ROOT_DATA_DIR_RT}/var/etc/security/join.key"
        if [ ! -f "${joinKeyFile}" ] && [ ! -z "${JF_SHARED_SECURITY_JOINKEY}" ]; then
            createRecursiveDir "${JF_ROOT_DATA_DIR_RT}" "var/etc/security" "${DOCKER_USER_RT}" "${DOCKER_USER_RT}"
            echo -n "${JF_SHARED_SECURITY_JOINKEY}" > "${joinKeyFile}" || warn "Could not create ${joinKeyFile}"
            io_setOwnership "${joinKeyFile}" "${DOCKER_USER_RT}" "${DOCKER_USER_RT}" || warn "Setting ownership of [${joinKeyFile}] to [${DOCKER_USER_RT}:${DOCKER_USER_RT}] failed" 
        fi
    fi
}
...
docker_createInstallerStateYaml(){
    ...
    if [[ ${TRIAL_FLOW} == ${FLAG_Y} ]]; then
        INSTALLER_STATE_YAML_FILE_RT="${JF_ROOT_DATA_DIR_RT}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}"
        INSTALLER_STATE_YAML_FILE_XRAY="${JF_ROOT_DATA_DIR_XRAY}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}"
        installerYamlFiles="$INSTALLER_STATE_YAML_FILE_RT $INSTALLER_STATE_YAML_FILE_XRAY"
    else
        INSTALLER_STATE_YAML_FILE="${JF_ROOT_DATA_DIR}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}"
        installerYamlFiles="$INSTALLER_STATE_YAML_FILE"
    fi

    for installerYamlFile in ${installerYamlFiles}; do
     [ ! -z "${installerYamlFile}" ] || return 0
        setInstallerStateKeys
        for stateIndicatorKey in ${INSTALLER_STATE_KEYS}; do
            eval stateIndicatorValue=\$${stateIndicatorKey}
            [ ! -z "${stateIndicatorValue}" ] || continue
            docker_installToSystemYaml "$stateIndicatorValue" "$FLAG_NOT_APPLICABLE" "$stateIndicatorValue" "${installerYamlFile}" "NOT_SET" \
                || logSilly "Key: $stateIndicatorKey not applicable"
        done
    done

    if [[ ${TRIAL_FLOW} == ${FLAG_Y} ]]; then
        io_setOwnership "${INSTALLER_STATE_YAML_FILE_RT}" "$DOCKER_USER_RT" "$DOCKER_USER_RT"
        io_setOwnership "${INSTALLER_STATE_YAML_FILE_XRAY}" "$DOCKER_USER_XRAY" "$DOCKER_USER_XRAY"
    else
...
docker_setUpPostgresCompose() {
    ...
    if [[ "$IS_POSTGRES_REQUIRED" == "$FLAG_Y" ]]; then
        ...
        if [[ ${TRIAL_FLOW} == ${FLAG_Y} ]]; then
            docker_getSystemValue "$SYS_KEY_SHARED_DATABASE_PASSWORD" "$FLAG_NOT_APPLICABLE" "$SYSTEM_YAML_FILE_RT"
        else
        ...
        if [[ "${OS_TYPE}" == "Darwin" ]]; then
            if [[ "${PRODUCT_NAME}" == "$ARTIFACTORY_LABEL" && "${TRIAL_FLOW}" == "$FLAG_Y" ]]; then
...
_backupFile() {
    ...
    if [ -f "${sourceFile}" ]; then
        ...
        io_setOwnership "${destinationFile}" "${dockerUser}" "${dockerUser}" || warn "Setting ownership of [${destinationFile}] to [${dockerUser}:${dockerUser}] failed"
    fi
}
...
docker_cleanup () {
    ...
    local dockerUser="$2"
    wrapper_removeInstallerYaml
    _backupFile "${SCRIPT_HOME}/config.log" "${rootDir}/var/log/config.log" "${dockerUser}"
    _backupFile "${SCRIPT_HOME}/bin/migration.log" "${rootDir}/var/log/migration.log" "${dockerUser}"
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        yes | \cp  -f "${SCRIPT_HOME}"/templates/${PROJECT_ROOT_FOLDER_RT}/system*.yaml  "${JF_ROOT_DATA_DIR_RT}/var/etc/"
        yes | \cp  -f "${SCRIPT_HOME}"/templates/${PROJECT_ROOT_FOLDER_XRAY}/system*.yaml  "${JF_ROOT_DATA_DIR_XRAY}/var/etc/"
    else
        yes | \cp  -f "${SCRIPT_HOME}"/templates/system*.yaml  "${rootDir}/var/etc/"
    fi
    local destinationFile="${rootDir}/var/etc/system.basic-template.yaml"
    io_setOwnership "$destinationFile" "${dockerUser}" "${dockerUser}" || warn "Setting ownership of [${destinationFile}] to [${dockerUser}:${dockerUser}] failed"
    
    destinationFile="${rootDir}/var/etc/system.full-template.yaml"
    io_setOwnership "$destinationFile" "${dockerUser}" "${dockerUser}" || warn "Setting ownership of [${destinationFile}] to [${dockerUser}:${dockerUser}] failed"
}
...
seedPostgresRtXrayDb() {
    ...
    [ -f "${THIRDPARTY_HOME}/postgresql/createPostgresUsers.sh" ] || errorExit "Could not find the following file : ${THIRDPARTY_HOME}/postgresql/createPostgresUsers.sh ,this is used to create postgres database, users and schema"
    while [[ $n -lt $max && "$SUCCESS" == "false" ]] ; do
        docker cp ${THIRDPARTY_HOME}/postgresql/createPostgresUsers.sh postgresql:/  && SUCCESS=true || SUCCESS=false
        ((n++))
        [[ $n -lt $max && "$SUCCESS" == "false" ]] && logger "Retrying copy of ${THIRDPARTY_HOME}/postgresql/createPostgresUsers.sh to $POSTGRES_LABEL container" || true
        sleep $delay;
    done
    ...
    docker exec -t postgresql bash -c "export DB_PASSWORD=${POSTGRES_PASSWORD} && chmod +x /createPostgresUsers.sh && su-exec postgres /createPostgresUsers.sh"  || errorExit "Failed to create $POSTGRES_LABEL default users"
}

docker_configureLogRotation() {
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    local rootDir="$1"
    local projectRootFolder="$2"
    local dockerUser="$3"
    _addLogRotateBinary "$rootDir" || true # Intentional. We want to still try and setup logrotate
    configureLogRotation "${projectRootFolder}" "${rootDir}" "${dockerUser}" "${dockerUser}" "$(id -un)" || true
}
...
wrapper_hook_start_getUserInputs(){
    ...
    JF_ROOT_DATA_DIR_RT="${JF_ROOT_DATA_DIR}/${PROJECT_ROOT_FOLDER_RT}"
    JF_ROOT_DATA_DIR_XRAY="${JF_ROOT_DATA_DIR}/${PROJECT_ROOT_FOLDER_XRAY}"
}
...
docker_getUserInputs(){
    ...
    if [[ ${TRIAL_FLOW} == ${FLAG_Y} ]]; then
        if [[ "${DOCKER_DESKTOP_SETUP}" == "$FLAG_Y" ]]; then
            docker_addToEnvFile "JF_SHARED_NODE_IP" "${JFROG_HOST_DOCKER_INTERNAL}"
        else
            docker_addToEnvFile "JF_SHARED_NODE_IP" "$(wrapper_getHostIP)"
        fi
    fi
...
docker_setInstallerStateFile(){
    if [[ ${TRIAL_FLOW} == ${FLAG_Y} ]]; then
        [ ! -z "${JF_ROOT_DATA_DIR_RT}" ]  &&  INSTALLER_STATE_YAML_FILE_RT="${JF_ROOT_DATA_DIR_RT}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}" || true
        [ ! -z "${JF_ROOT_DATA_DIR_XRAY}" ]  &&  INSTALLER_STATE_YAML_FILE_XRAY="${JF_ROOT_DATA_DIR_XRAY}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}" || true
    else
        [ ! -z "${JF_ROOT_DATA_DIR}" ] && INSTALLER_STATE_YAML_FILE="${JF_ROOT_DATA_DIR}/var/etc/${FILE_NAME_INSTALLER_STATE_YAML}" || true
    fi
}
...
docker_main() {
    ...
    if [ "$(io_commandExists 'jq')" != "yes" ]; then
        errorExit "Jq is mandatory to get latest version of jfrog xray compose" 
        return
    fi

    if [ "$(io_commandExists 'curl')" != "yes" ]; then
        errorExit "Curl is mandatory to download latest version of jfrog xray compose" 
        return
    fi
    ...
    docker_setInstallerStateFile
    _pauseExecution "docker_setInstallerStateFile"
    ...
    bannerStart "Beginning ${PRODUCT_NAME} setup"
   ...
   # At this point, we know what the user wants. Set up variables
    docker_setupModeSpecificVar
    _pauseExecution "docker_setupModeSpecificVar"
    ...
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        docker_createDirectories "${JF_ROOT_DATA_DIR_RT}" "${DOCKER_USER_RT}"
        _pauseExecution "docker_createDirectories"
        docker_createDirectories "${JF_ROOT_DATA_DIR_XRAY}" "${DOCKER_USER_XRAY}"
        _pauseExecution "docker_createDirectories"
    else
        docker_createDirectories "${JF_ROOT_DATA_DIR}" "${DOCKER_USER}"
        _pauseExecution "docker_createDirectories"
    fi

    #setup permissions on the root folder
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        docker_setRootDirPermissions  "${JF_ROOT_DATA_DIR_RT}" "${DOCKER_USER_RT}"
        _pauseExecution "docker_setRootDirPermissions"
        docker_setRootDirPermissions "${JF_ROOT_DATA_DIR_XRAY}" "${DOCKER_USER_XRAY}"
        _pauseExecution "docker_setRootDirPermissions"
    else
        docker_setRootDirPermissions "${JF_ROOT_DATA_DIR}" "${DOCKER_USER}"
        _pauseExecution "docker_setRootDirPermissions"
    fi
    ...
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        docker_createSystemYaml ${SYSTEM_YAML_FILE_RT}
        _pauseExecution "docker_createSystemYaml"
        docker_createSystemYaml ${SYSTEM_YAML_FILE_XRAY}
        _pauseExecution "docker_createSystemYaml"
    else
        docker_createSystemYaml ${SYSTEM_YAML_FILE}
        _pauseExecution "docker_createSystemYaml"
    fi
    ...
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        dockerCreateJoinKeyInRT
        _pauseExecution "dockerCreateJoinKeyInRT"
    fi
    ...
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        docker_configureLogRotation "${JF_ROOT_DATA_DIR_RT}" "${PROJECT_ROOT_FOLDER_RT}" "${DOCKER_USER_RT}"
        _pauseExecution "docker_configureLogRotation"
        docker_configureLogRotation "${JF_ROOT_DATA_DIR_XRAY}" "${PROJECT_ROOT_FOLDER_XRAY}" "${DOCKER_USER_XRAY}"
        _pauseExecution "docker_configureLogRotation" 
    else
        docker_configureLogRotation "${JF_ROOT_DATA_DIR}" "${PROJECT_ROOT_FOLDER}" "${DOCKER_USER}"
        _pauseExecution "docker_configureLogRotation"
    fi
    ...
    #backup the env file
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        docker_cleanup "${JF_ROOT_DATA_DIR_RT}" "${DOCKER_USER_RT}"
        docker_cleanup "${JF_ROOT_DATA_DIR_XRAY}" "${DOCKER_USER_XRAY}"
    else
        docker_cleanup "${JF_ROOT_DATA_DIR}" "${DOCKER_USER}"
    fi
